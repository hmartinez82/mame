...
void specnext_state::tbblue(machine_config &config)
{
	...
	DAC_8BIT_R2R(config, m_dac[0], 0).add_route(ALL_OUTPUTS, "speaker", 0.75, 0);
	DAC_8BIT_R2R(config, m_dac[1], 0).add_route(ALL_OUTPUTS, "speaker", 0.75, 0);
	DAC_8BIT_R2R(config, m_dac[2], 0).add_route(ALL_OUTPUTS, "speaker", 0.75, 1);
	DAC_8BIT_R2R(config, m_dac[3], 0).add_route(ALL_OUTPUTS, "speaker", 0.75, 1);

	config.device_remove("ay8912");
	for (auto i = 0; i < 3; ++i)
	{
		YM2149(config, m_ay[i], 14_MHz_XTAL / 8)
			.add_route(0, "speaker", 0.50, 0)
			.add_route(1, "speaker", 0.25, 0)
			.add_route(1, "speaker", 0.25, 1)
			.add_route(2, "speaker", 0.50, 1);
	}
